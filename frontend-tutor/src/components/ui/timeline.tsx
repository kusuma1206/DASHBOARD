...
export const Timeline = ({
  ...
  return (
    ...
    >
      ...
      <div ref={ref} className="relative max-w-[1400px] mx-auto pb-20 px-6 md:px-12">
        {data.map((item, index) => (
          <motion.div
            key={index}
            ...
            className="flex flex-col md:grid md:grid-cols-[100px_1fr_1fr] gap-8 md:gap-16 pt-10 md:pt-32"
          >
            {/* Column 1: Progress Line & Dot */}
            ...
            <div className="flex flex-col items-start text-left">
              <div className="px-2 py-0.5 bg-white border border-[#1E3A47]/10 rounded-md mb-4 inline-block shadow-sm">
                <span className="text-[9px] font-black uppercase tracking-[0.2em] text-[#1E3A47]/40">{item.badge}</span>
              </div>
              <h3 className="text-4xl md:text-5xl font-black text-[#1E3A47] tracking-tight mb-1">
                {index + 1}<span className="text-[#B24531]">.</span>
              </h3>
              <h4 className="text-xl md:text-2xl font-black text-[#1E3A47] mb-4 leading-tight">
                {item.title}
              </h4>
              <p className="text-[#1E3A47]/70 text-sm md:text-base font-medium leading-relaxed max-w-sm">
                {item.description}
              </p>
            </div>
        ...
        <div
          ...
          className="absolute left-[calc(1.5rem+32px)] md:left-[calc(3rem+50px)] top-0 overflow-hidden w-[2px] bg-neutral-200 [mask-image:linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)]"
        >
          <motion.div
            style={{
